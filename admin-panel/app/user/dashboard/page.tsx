...
export default function UserDashboard() {
  ...
  return (
    <div className="space-y-8 max-w-[1200px] mx-auto pb-12">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Ana Sayfa</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">{taslak?.firma_adi || user?.firmaAdi || 'Yeni şirket'} hesabına hoş geldiniz.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          { title: 'Toplam Alacak', value: `₺${fmt(toplamAlacak)}`, note: toplamAlacak > 0 ? `${(taslak?.bekleyen_tahsilatlar || []).length} bekleyen tahsilat` : 'Bekleyen tahsilat yok', icon: TrendingDown, tone: 'text-red-600', border: 'border-l-red-400' },
          { title: 'Bu Ay Tahsilat', value: `₺${fmt(buAyTahsilat)}`, note: buAyTahsilat > 0 ? `${(taslak?.yapilan_tahsilatlar || []).length} ödeme yapıldı` : 'Henüz ödeme yok', icon: CheckCircle2, tone: 'text-emerald-600', border: 'border-l-emerald-500' },
          { title: 'Aktif Proje', value: String(aktifProje), note: aktifProje > 0 ? `${aktifProje} proje devam ediyor` : 'Aktif proje yok', icon: Briefcase, tone: 'text-blue-600', border: 'border-l-blue-500' },
          ...
          <Link key={item.href} href={item.href}>
            <Card className={`cursor-pointer border-2 ${item.renk} transition-all duration-200 hover:shadow-md group h-full`}>
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0"><p className="font-semibold text-sm text-slate-900 dark:text-white leading-tight">{item.label}</p></div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800 pb-3">
          ...
          <CardDescription>{chart.length > 0 ? `${taslak?.donem || '2024'} dönemsel verileri` : 'Henüz finansal veri girilmedi.'}</CardDescription>
        </CardHeader>
          ...
          {chart.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chart} barGap={6}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12 }} tickFormatter={v => `₺${(Number(v)/1e6).toFixed(1)}M`} />
            ...
            <div className="h-full flex items-center justify-center rounded-lg border border-dashed text-sm text-slate-500">Finansal veri eklendiğinde grafik burada görünecek.</div>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="p-6">
          {sonTahsilatlar.length > 0 ? (
            sonTahsilatlar.map(t => (
              <div key={t.aciklama + t.tarih} className="flex items-center justify-between py-3 border-b last:border-b-0">
                <div>
                  <p className="font-medium text-sm">{t.aciklama}</p>
                  <p className="text-xs text-slate-500">{t.tarih} — ₺{fmt(t.tutar)}</p>
                </div>
                <Badge variant="outline" className={t.durum === 'Ödendi' ? 'border-emerald-200 text-emerald-700 bg-emerald-50' : t.durum === 'Gecikti' ? 'border-red-200 text-red-700 bg-red-50' : 'border-amber-200 text-amber-700 bg-amber-50'}>{t.durum}</Badge>
              </div>
            ))
          ) : (
