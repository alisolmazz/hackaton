...
const DATA: { ay: string; giris: number; cikis: number; net: number }[] = [];
    ...
    <div className="space-y-8 max-w-[1200px] mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Bu şirket için henüz nakit akışı verisi girilmedi.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="shadow-sm border-l-4 border-l-emerald-500"><CardContent className="p-5">
          <div className="flex justify-between items-start">
            <div><p className="text-sm text-slate-500 font-medium">Net Nakit Pozisyon</p><p className="text-2xl font-bold mt-1 text-emerald-600">₺{fmt(netPoz)}</p></div>
        ...
        <CardContent className="pt-6 h-[380px]">
          {DATA.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center rounded-lg border border-dashed text-sm text-slate-500">
              Nakit girişi ve çıkışı girildiğinde grafik burada görünecek.
            </div>
          )}
        </CardContent>
      </Card>
