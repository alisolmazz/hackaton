...
export function EmptyState({ icon: Icon, title, message, description, actionLabel, onAction, variant = 'default' }: EmptyStateProps) {
  const styles = VARIANT_STYLES[variant];
  const displayTitle = title || message || 'Veri bulunamadı';
  const DisplayIcon = Icon || FileSearch;
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className={`w-16 h-16 rounded-full ${styles.iconBg} flex items-center justify-center mb-4`}>
        <DisplayIcon className={`w-8 h-8 ${styles.iconColor}`} />
      </div>
      <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">{displayTitle}</h3>
